...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (route[0] === "orders") return run(req, res, () => import("../../server/api/pool/orders.js"));

    if (route[0] === "import-batches") {
    ...
    return res.status(404).json({ ok: false, error: "Route not found", path: `pool/${path}` });
  } catch (err: any) {
